...
with open(f"../run_parameters/pseudonam_{args.dataset_name}_seed{args.seed}_params{param_id}.yaml", "w") as f:
...
torch.save(
    model, 
    f"../model_saves/pseudoanm_{args.dataset_name}_seed{args.seed}.pt"
)
...
cdf_preds = 1 / (1 + np.exp(-1 * model.predict(X_test)))
...
results = pd.DataFrame(
    [["pseudonam", args.dataset_name, args.seed, cindex, mean_auc, brier_score, param_id]],
    columns=["model", "dataset", "seed", "cindex", "mean_auc", "brier_score", "param_id"]
)

results.to_csv(
    f"../metric_saves/pseudonam_{args.dataset_name}_seed{args.seed}.csv", index=False
)